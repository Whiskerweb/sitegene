...
export async function chat(messages: Msg[], opts?: { json?: boolean; maxTokens?: number; temperature?: number }): Promise<string> {
  ...
  const res = await fetch(MISTRAL_URL, {
    ...
    body: JSON.stringify({
      ...
      // 0.2 par défaut (édition précise) ; les usages créatifs (chartes) montent.
