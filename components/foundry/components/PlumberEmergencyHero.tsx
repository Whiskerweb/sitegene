    ...
    <section className="pehero relative overflow-hidden px-6 pb-20 pt-12" style={root}>
      ...
      <div className="pointer-events-none absolute right-0 top-0 h-full w-2/3" aria-hidden>
        <svg viewBox="0 0 600 600" className="h-full w-full opacity-40" preserveAspectRatio="none">
          <path d="M600 0 C 400 120, 520 320, 600 420 L 600 0 Z" fill="var(--c-accent)" />
        </svg>
      </div>
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-10 md:grid-cols-2">
        ...
        <div className="pehero-in">
          <h1 className="mb-6 text-4xl font-bold leading-tight md:text-5xl lg:text-6xl" style={{ fontFamily: "var(--font-heading)", color: "var(--c-ink)" }}>
            {content?.title ?? "Tous vos travaux de plomberie, partout chez vous."}
          </h1>
          <p className="mb-8 max-w-md text-lg" style={{ color: "var(--c-muted)" }}>
          ...
          <a href={content?.ctaHref || "#contact"} className="inline-flex items-center gap-3 rounded-[var(--r-card)] px-7 py-4 text-sm font-bold shadow-lg transition-all hover:brightness-105" style={{ background: "var(--c-accent)", color: "var(--c-on-accent)" }}>
          ...
          <p className="mt-4 text-xs" style={{ color: "var(--c-muted)" }}>{content?.availability ?? "Disponible 24/7, appelez à tout moment"}</p>
        </div>
        ...
        <div className="pehero-in2 relative">
          ...
          <div className="absolute bottom-6 left-6 flex items-center gap-3 rounded-[var(--r-card)] px-5 py-3 shadow-xl" style={{ background: "var(--c-card)" }}>
