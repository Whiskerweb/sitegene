...
export default function FxShuffleReviews({ content }: { content: FxShuffleReviewsContent; skin: Skin }) {
  ...
  return (
    <section className="grid place-content-center overflow-hidden px-8 py-20" style={{ background: "color-mix(in srgb, var(--c-accent) 14%, #0d0d10)", minHeight: "34rem" }}>
        ...
        {order.map((idx, pos) => {
          ...
          return (
            ...
            >
              ...
              <figcaption className="text-center text-sm font-medium not-italic" style={{ color: "rgba(255,255,255,.9)" }}>{it.author}</figcaption>
            </figure>
          );
        })}
      </div>
