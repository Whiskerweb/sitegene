...
export default function FxDisplayCards({ content }: { content: FxDisplayCardsContent; skin: Skin }) {
  ...
  return (
    <section className="overflow-hidden px-5 py-16 md:py-24" style={{ background: "var(--c-panel)" }}>
      <div className="mx-auto max-w-[1100px]">
        <div className="text-center">
          <p className="text-xs font-semibold uppercase tracking-[0.22em]" style={{ color: "var(--c-accent)" }}>{content.eyebrow}</p>
          <h2 className="mx-auto mt-4 max-w-xl text-[2rem] md:text-[2.6rem]" style={{ fontFamily: "var(--font-heading)", color: "var(--c-on-panel)", letterSpacing: "-1.2px", lineHeight: 1.1 }}>
            {content.title}
          </h2>
        </div>
      ...
      <style>{`
        .sg-fx-dc{position:relative;display:flex;height:9rem;width:22rem;box-sizing:border-box;transform:skewY(-8deg);
          ...
          border:2px solid rgba(255,255,255,.08);background:color-mix(in srgb,var(--c-panel) 60%,#fff 6%);
        ...
        .sg-fx-dc::after{content:'';position:absolute;right:-.25rem;top:-5%;height:110%;width:20rem;
          background:linear-gradient(to left,var(--c-panel),transparent);pointer-events:none;z-index:2}
        ...
        .sg-fx-dc-icon{display:inline-grid;place-items:center;border-radius:9999px;width:1.7rem;height:1.7rem;flex:none;
          background:color-mix(in srgb,var(--c-accent) 30%,var(--c-panel));color:var(--c-accent2)}
        .sg-fx-dc-title{font-size:1.05rem;font-weight:600;margin:0;color:var(--c-on-panel)}
        .sg-fx-dc-desc{font-size:1.02rem;color:var(--c-on-panel);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;position:relative;z-index:1;margin:0}
        ...
        .sg-fx-dc-1::before,.sg-fx-dc-2::before{content:'';position:absolute;inset:0;border-radius:inherit;
          background:color-mix(in srgb,var(--c-panel) 50%,transparent);opacity:1;transition:opacity .7s;z-index:2}
