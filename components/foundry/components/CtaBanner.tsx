...
export default function CtaBanner({ content }: { content: CtaContent; skin: Skin }) {
  return (
    <section className="px-5 py-8" style={{ background: "var(--c-surface)" }}>
      <div className="relative mx-auto max-w-[1280px] overflow-hidden rounded-[var(--r-xl)]" style={{ background: "var(--c-panel)" }}>
        {content.image && (
          <>
            <img src={content.image} alt="" aria-hidden className="absolute inset-0 h-full w-full object-cover opacity-25" />
            <div className="absolute inset-0" style={{ background: "color-mix(in srgb, var(--c-panel) 78%, transparent)" }} />
          </>
        )}
        <div className="relative z-10 mx-auto max-w-2xl px-6 py-20 text-center md:py-28">
          <h2
            ...
            style={{ fontFamily: "var(--font-heading)", color: "var(--c-on-panel)", letterSpacing: "-1px", lineHeight: 1.15 }}
          >
            {content.title}
          </h2>
          <a
