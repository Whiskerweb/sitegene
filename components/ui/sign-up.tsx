...
import { ArrowRight, Mail, Gem, X, AlertCircle, PartyPopper, Loader } from "lucide-react";
...
const loadingSteps = ["Envoi du lien…", "Préparation…", "Presque prêt…"];
...
interface AuthComponentProps {
  ...
  brandName?: string;
  /** Si fourni, le logo + nom de marque deviennent un lien (ex. retour accueil). */
  homeHref?: string;
  /** Câblage Supabase : envoie le magic link. Retourne `{ error }` en cas d'échec. */
  onSubmitEmail: (email: string) => Promise<SubmitResult>;
  /** Optionnel : connexion OAuth (boutons Google / GitHub). Sinon ils sont masqués. */
  onOAuth?: (provider: OAuthProvider) => Promise<SubmitResult>;
  /** Erreur initiale (ex. lien expiré, accès refusé) passée par la page. */
  initialError?: string;
  title?: string;
  subtitle?: string;
  /** Fond personnalisé : remplace le GradientBackground SVG par défaut. */
  background?: React.ReactNode;
  /** Styles inline sur le conteneur racine (ex. variables de thème claires). */
...
export const AuthComponent = ({
  ...
  useEffect(() => {
    if (initialError) {
      setModalErrorMessage(initialError);
  ...
  const submitEmail = useCallback(async () => {
    if (!isEmailValid || modalStatus !== "closed") return;
  ...
  }, [email, isEmailValid, modalStatus, onSubmitEmail]);

  const handleOAuth = useCallback(
    async (provider: OAuthProvider) => {
      if (!onOAuth || modalStatus !== "closed") return;
      const res = await onOAuth(provider);
      if (res && res.error) {
        setModalErrorMessage(res.error);
        setModalStatus("error");
      }
      // succès = redirection navigateur gérée par Supabase
    },
    [onOAuth, modalStatus],
  );
          ...
          >
            ...
            {modalStatus === "success" && (
              <div className="flex flex-col items-center gap-4 text-center">
                ...
                <p className="text-lg font-medium text-foreground">Vérifiez vos emails</p>
                <p className="text-sm text-muted-foreground">
                  Lien de connexion envoyé à <span className="text-foreground">{sentTo}</span>.
                </p>
              </div>
            )}
          </motion.div>
  ...
  return (
    <div className="bg-background min-h-screen w-screen flex flex-col" style={rootStyle}>
      <style>{`
      ...
      {homeHref ? (
        <Link
          ...
          className={cn(
            "fixed top-4 left-4 z-20 flex items-center gap-2 rounded-full transition-opacity hover:opacity-80",
      ...
      <div className={cn("flex w-full flex-1 h-full items-center justify-center", background ? "" : "bg-card", "relative overflow-hidden")}>
        ...
        <fieldset disabled={modalStatus !== "closed"} className="relative z-10 flex flex-col items-center gap-8 w-[300px] mx-auto p-4">
          ...
          >
            <BlurFade delay={0.25 * 1} className="w-full">
              <div className="text-center">
                <p
                  ...
                  style={background ? { textShadow: "0 2px 24px rgba(10,25,45,0.55)" } : undefined}
                >
                  {title}
                </p>
              </div>
            ...
            <BlurFade delay={0.25 * 2} className="w-full">
              <p
                className="text-sm font-medium text-muted-foreground text-center text-balance"
                style={background ? { textShadow: "0 1px 14px rgba(10,25,45,0.5)" } : undefined}
              >
                {subtitle}
              </p>
            </BlurFade>
              ...
              <>
                <BlurFade delay={0.25 * 3}>
                  <div className="flex items-center justify-center gap-4 w-full">
                    <GlassButton type="button" onClick={() => handleOAuth("google")} contentClassName="flex items-center justify-center gap-2" size="sm">
                      <GoogleIcon />
                      <span className="font-semibold text-foreground">Google</span>
                    </GlassButton>
                    <GlassButton type="button" onClick={() => handleOAuth("github")} contentClassName="flex items-center justify-center gap-2" size="sm">
                      <GitHubIcon />
                ...
                <div className="glass-input-wrap w-full">
                  <div className="glass-input">
                    ...
                    <input
                      ...
                      className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none px-2"
                    />
                    <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isEmailValid ? "w-10 pr-1" : "w-0")}>
                      <GlassButton
                        type="submit"
                        size="icon"
                        aria-label="Recevoir mon lien de connexion"
