...
export function HeroWave({
  ...
  const basePlaceholder = "Décrivez votre univers : ";
  const suggestionsRef = useRef<string[]>([
    "Camille, photographe portrait à Lyon. Lumière douce...",
    "Atelier Horizon, photo d'architecture minimaliste...",
    "Studio Nuit, packshot noir et blanc contrasté...",
    "Léo, photographe mariage et moments volés...",
  ]);
  ...
  useEffect(() => {
    ...
    const pauseAtEnd = 1500; // pause after full word
    const pauseBetween = 400; // pause after clearing
  ...
  useEffect(() => {
    ...
    function createInstancedMaterial() {
      const baseCol = new THREE.Color("hsl(262,100%,70%)"); // customized to match the violet-400 theme of Akyra
      const emisCol = new THREE.Color("#6d4aff"); // electric violet accent
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Badge discret : prix fixe + accroche rotative (façon Aura/Apple) */}
          <div className="relative inline-block mb-7">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/[0.04] px-3.5 py-1.5 text-[12px] font-medium text-muted border border-white/10 backdrop-blur-sm">
              <span className="font-semibold text-paper">À partir de 50 €/an</span>
          ...
          {/* Titre court, 1 ligne, pas trop gros */}
          <h1 className="text-white font-display text-3xl sm:text-5xl font-bold leading-[1.05] tracking-[-0.03em] whitespace-normal sm:whitespace-nowrap">
            {title}{" "}
            <CircledText
              stroke="var(--color-gold-400)"
              delay={0.7}
              className="text-violet-400 font-display"
            >
              déjà construit.
            </CircledText>
          </h1>

          {/* Description courte + lien discret */}
          <p className="text-muted mt-4 text-[15px] sm:text-[17px] leading-relaxed">
            {subtitle}{" "}
            <a
              href="#templates"
              className="whitespace-nowrap font-medium text-paper underline decoration-white/30 underline-offset-4 transition hover:decoration-white"
            >
              Voir un exemple.
            </a>
          </p>
          ...
          <form
            className="mt-6 sm:mt-8 flex items-center justify-center relative"
            onSubmit={(e) => {
              e.preventDefault();
              onPromptSubmit?.(prompt, photos);
            }}
          >
            <div className="relative w-full sm:w-[720px] text-left">
              {/* Annotation manuscrite près du bloc de saisie */}
              <div className="absolute -right-32 -top-12 hidden rotate-6 select-none font-hand text-[22px] text-gold-400 md:block">
                en 30 secondes ⏱ ──┐
              </div>

              {/* Glowing, glassmorphic prompt input block */}
              <div className="relative rounded-[20px] p-[1px] shadow-[0_25px_60px_-15px_rgba(109,74,255,0.25)] bg-white/10 focus-within:bg-violet-500/30 transition duration-300">
                <div className="rounded-[19px] bg-ink-800/80 backdrop-blur-xl border border-white/5 overflow-hidden">
                  <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder={animatedPlaceholder}
                    className="w-full h-24 sm:h-28 resize-none bg-transparent text-white placeholder:text-faint outline-none px-4 pt-4 text-[15px] leading-relaxed"
                  />
                  
                  {/* Action Bar (Identical to Aura.build) */}
                  <div className="flex items-center justify-between px-4 pb-3.5 pt-2 border-t border-white/5 flex-wrap gap-2">
                    {/* Left : dépôt de photos (réel) */}
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <button
                        ...
                        className="cursor-pointer text-[11px] font-semibold text-muted hover:text-white transition px-2.5 py-1 rounded-xl bg-ink-700/60 border border-white/5 flex items-center gap-1"
                      >
                        📎 {photos.length > 0 ? `${photos.length} photo${photos.length > 1 ? "s" : ""}` : "Ajouter des photos"}
                      </button>
                      {photos.length > 0 && (
                        <button
                          type="button"
                          onClick={() => setPhotos([])}
                          className="text-[11px] font-medium text-faint hover:text-white transition"
                        >
                          retirer
                        </button>
                      )}
              ...
              {/* Social proof discret, sous le champ */}
              <p className="mt-4 text-center text-[12.5px] text-faint">
                Déjà adopté par plus de <span className="font-semibold text-muted">2 400 créateurs</span>.
              </p>
            </div>
          </form>
        </div>
