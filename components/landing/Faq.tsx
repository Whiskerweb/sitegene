        ...
        >
          {faqs.map((f) => (
            <motion.details
              key={f.q}
              variants={fadeUp}
              className="group rounded-[18px] border border-sky-200 bg-surface-2 px-6 [&_summary::-webkit-details-marker]:hidden"
            >
              <summary className="flex cursor-pointer items-center justify-between gap-4 py-5 text-[17px] font-medium text-night">
                {f.q}
                <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full border border-sky-300 text-lg text-slate transition-all duration-300 group-open:rotate-45 group-open:border-brand group-open:text-brand">
                  +
                </span>
              </summary>
